...
\pagebreak
\section{Analysis of Data from Nielsen et al.}
In \cite{detection_artmann_submitted}, we analysed the neurogenesis data from Nielsen et al. We use part of this dataset as an example on how to analyse data with `miRtest'.\\
...
P = miR.test(X=X,Y=Y,A=A,group.mRNA=group.mRNA,group.miRNA=group.miRNA,adjust="BH",gene.set.tests="all",verbose=TRUE)
@
